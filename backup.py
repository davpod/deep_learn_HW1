...
K_FOLDS = 5
DROPOUT_PROB = 0.0
TRAIN_SIZE = 250  # subset for faster training
...
class SmallCNN(nn.Module):
    def __init__(self, num_classes=NUM_CLASSES, dropout_prob=DROPOUT_PROB):
        ...
        self.fc1 = nn.Linear(512*(INPUT_SIZE//16)*(INPUT_SIZE//16), 512)
...
test_dataset = CassavaDataset("train.csv", img_dir, transform=test_transform)
...
for fold, (train_idx, val_idx) in enumerate(kfold.split(trainval_idx, labels)):
    ...
    for epoch in range(NUM_EPOCHS):
        # Train
        model.train()
        correct, total, loss_accum = 0, 0, 0
